...
window.onload = () => {
  ...
  apiClient.get("/api/get-recruiters-list/", {
    ...
    if (profileData) {
      var profileDataObject = JSON.parse(profileData);

      // Access the candidateData attribute within the profileData object
      candidateData = profileDataObject.candidateData;
      console.log("ccccccccd", candidateData);
      notes = profileDataObject.Notes;
      Object.freeze(candidateData);
      console.log("candidateData on load===", candidateData);
...
function populateFormFields(candidateData) {
  if (candidateData) {
    document.getElementById("fullName").value = candidateData.candidateData.CandidateInfo.FullName;
    document.getElementById("jobTitle").value = candidateData.candidateData.CandidateInfo["Job Title"];
    document.getElementById("phoneNumber").value = candidateData.candidateData.CandidateInfo.PhoneNumber.FormattedNumber;
    document.getElementById("location").value = candidateData.candidateData.CandidateInfo.PhoneNumber.Location;
    document.getElementById("emailAddress").value = candidateData.candidateData.CandidateInfo.Email;
    ...
    document.getElementById("linkedIn").value = candidateData.candidateData.CandidateInfo.Linkedin;
    document.getElementById("gitHub").value = candidateData.candidateData.CandidateInfo.Github;
    document.getElementById("links").value = candidateData.candidateData.CandidateInfo.Links.join(', ');
    document.getElementById("country").value = candidateData.candidateData.CandidateInfo.Country;
    document.getElementById("nationality").value = candidateData.candidateData.CandidateInfo.Nationality;
    document.getElementById("dateOfBirth").value = candidateData.candidateData.CandidateInfo.DateOfBirth;
    document.getElementById("gender").value = candidateData.candidateData.CandidateInfo.Gender;
    document.getElementById("maritalStatus").value = candidateData.candidateData.CandidateInfo.MaritalStatus;
    document.getElementById("status").value = candidateData.status;
    skills = {
    ...
    certifications = candidateData.candidateData.CandidateInfo.Certifications;
    ...
    function renderDegrees() {
      ...
      degrees.forEach((degree, index) => {
        ...
        startDateInput.value = degree.StartDate;
    ...
    function displayDegreesForm() {
      ...
      normalizeDegreeInput.className = "block w-full mt-2 p-2 border rounded";
      formContainer.appendChild(normalizeDegreeInput);
      const specializationInput = document.createElement("input");
      specializationInput.type = "text";
      specializationInput.placeholder = "Specialization";
      specializationInput.className = "block w-full mt-2 p-2 border rounded";
      formContainer.appendChild(specializationInput);
      const countryOrInstituteInput = document.createElement("input");
      countryOrInstituteInput.type = "text";
      countryOrInstituteInput.placeholder = "Country Or Institute";
      countryOrInstituteInput.className = "block w-full mt-2 p-2 border rounded";
    ...
    function renderExperiences() {
      ...
      experiences.forEach((exp, index) => {
        ...
        periodeLabel.className = "block w-1/3 text-sm font-medium text-black dark:text-white mr-2";
        ...
        periodeInput.className = "w-full rounded border border-stroke bg-gray px-3 py-2 font-medium text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    ...
    console.log("Form data saved:", Object.fromEntries(formData.entries()));
    ...
    const experience = candidateData.candidateData.CandidateInfo.Experience;
    const candidateDataUpdated = {};
    function normalizeValue(value) {
      return value === null ? "" : value;
    }
    if (normalizeValue(candidateData.availability) !== normalizeValue(availability)) {
      candidateDataUpdated.availability = availability;
      console.log("different avail");
    }
    if (normalizeValue(candidateData.mobility) !== normalizeValue(mobility)) {
      candidateDataUpdated.mobility = mobility;
    }
    if (normalizeValue(candidateData.recruiter) !== normalizeValue(recruiter)) {
    ...
    candidateData.candidateData.CandidateInfo["Job Title"] = jobTitle;
    ...
    candidateData.candidateData.CandidateInfo.PhoneNumber.FormattedNumber = phoneNumber;
    candidateData.candidateData.CandidateInfo.PhoneNumber.Location = location;
    ...
    candidateData.candidateData.CandidateInfo.MaritalStatus = maritalStatus;
    ...
    Object.keys(formEntries).forEach(key => {
      ...
      if (key.startsWith("certificationName")) {
        ...
        candidateData.candidateData.CandidateInfo.Certifications[index] = candidateData.candidateData.CandidateInfo.Certifications[index] || {};

        // Add the CertificationName field
        candidateData.candidateData.CandidateInfo.Certifications[index].CertificationName = formEntries[key];
      }
      ...
      if (key.startsWith("issuingOrganization")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Certifications[index] = candidateData.candidateData.CandidateInfo.Certifications[index] || {};
        candidateData.candidateData.CandidateInfo.Certifications[index].IssuingOrganization = formEntries[key];
      }
      if (key.startsWith("issueDate")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Certifications[index] = candidateData.candidateData.CandidateInfo.Certifications[index] || {};
        candidateData.candidateData.CandidateInfo.Certifications[index].IssueDate = formEntries[key];
      }
      if (key.startsWith("degreeName")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].DegreeName = formEntries[key];
      }
      if (key.startsWith("normalizeDegree")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].NormalizeDegree = formEntries[key];
      }
      if (key.startsWith("specialization")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].Specialization = formEntries[key];
      }
      if (key.startsWith("startDate")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].StartDate = formEntries[key];
      }
      if (key.startsWith("endDate")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].EndDate = formEntries[key];
      }
      if (key.startsWith("countryOrInstitute")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Degrees[index] = candidateData.candidateData.CandidateInfo.Degrees[index] || {};
        candidateData.candidateData.CandidateInfo.Degrees[index].CountryOrInstitute = formEntries[key];
      }
      if (key.startsWith("experienceTitle")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Experience[index] = candidateData.candidateData.CandidateInfo.Experience[index] || {};
        candidateData.candidateData.CandidateInfo.Experience[index].Title = formEntries[key];
      }
      if (key.startsWith("experiencePeriode")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Experience[index] = candidateData.candidateData.CandidateInfo.Experience[index] || {};
        candidateData.candidateData.CandidateInfo.Experience[index].Periode = formEntries[key];
      }
      if (key.startsWith("experienceDescription")) {
        const index = key.match(/\d+/)[0];
        candidateData.candidateData.CandidateInfo.Experience[index] = candidateData.candidateData.CandidateInfo.Experience[index] || {};
        candidateData.candidateData.CandidateInfo.Experience[index].Description = formEntries[key];
      }
    });
    console.log("candidateInfooooo===", candidateData.candidateData.CandidateInfo);
    ...
    if (fileInput.files.length > 0) {
      ...
      apiClient.patch(`/api/add-cv/${candidateData.id_candidate}/${idRecruiter}/`, {
      ...
      console.log("candidateDataUpdated===", candidateDataUpdated);
      apiClient.patch(`/api/update-cv/${candidateData.id_candidate}/`, candidateDataUpdated, {
        withCredentials: true,
        headers: {
...
function sendFileToServer(file) {
  ...
  apiClient.post(`/api/new-cv/${idRecruiter}/`, {
