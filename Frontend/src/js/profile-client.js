...
function toggleClientEditMode(clientId, editMode) {
  ...
  const editableFields = [
    ...
    'companyIndustry', 'companyDescription', 'companyPhone', 'companyURLs'
  ];
  if (editMode) {
...
function editClient(clientId) {
  ...
  const companyURLs = document.getElementById('companyURLs').value;
...
function load_Client() {
  ...
  // Correction ici : Nettoyer et formater les URLs
  ...
  if (urls && Array.isArray(urls)) {
    ...
    urls.forEach((url) => {
      if (url && typeof url === "string") {
        // Nettoyer les URLs des caractères indésirables
        let cleanedUrl = url
          .replace(/\\n/g, '') // Supprimer les sauts de ligne
          .replace(/\\xa0/g, ' ') // Remplacer les espaces insécables par des espaces normaux
          .replace(/[\[\]'"]+/g, '') // Supprimer les crochets et les guillemets
          .trim(); // Supprimer les espaces en début et fin de chaîne

        // Si l'URL nettoyée n'est pas vide, l'ajouter au tableau
        if (cleanedUrl && cleanedUrl !== "None") {
          cleanedUrls.push(cleanedUrl);
        }
      }
    });

    // Convertir le tableau nettoyé en une chaîne séparée par des virgules
    const urlsString = cleanedUrls.join(', ');

    // Récupérer l'élément pour afficher les URLs
    const urlsContainer = document.getElementById("companyURLs");
    if (urlsContainer) {
      // Afficher les URLs nettoyées dans un paragraphe
      urlsContainer.innerHTML = `<p>${urlsString}</p>`;
    }
  }
...
function addLead() {
  ...
  const notes = document.getElementById('leadNotes').value; // Add notes field
  ...
  const lead = {
    ...
    phone: phone,
    linkedIn: linkedIn,
    notes: notes, // Include notes in the lead object
    client_id: clientId,
  };
  ...
  const leadHTML = `
      <div data-id="${leadId}" class="mb-4 flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-4">
        <div class="flex-grow">
          ...
          <div class="mt-4">
            <label for="leadNotes-${lead.id}" class="block text-sm font-medium text-bodydark dark:text-white/70">Notes</label>
            <textarea id="leadNotes-${lead.id}" class="mt-1 block w-full border border-stroke rounded-lg p-2 text-bodydark dark:bg-boxdark dark:text-white">${lead.notes || ''}</textarea>
        </div>
        </div>
...
function editLead(leadId) {
  ...
  const notes = document.getElementById('leadNotes-' + leadId).value; // Get notes value
...
function load_Jobs() {
  ...
  function displayJobs(jobs) {
    ...
    jobs.forEach((job) => {
      const jobHTML = `
  <div data-id="${job.id}" class="job-card bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg p-4 mb-6 w-full flex flex-row items-center justify-between">
    <div class="flex-grow">
      ...
      <table class="w-full border-collapse">
        <tbody>
          ...
          <tr>
            <td class="font-semibold text-bodydark dark:text-white/70">Start Date:</td>
          ...
          <tr>
            <td class="font-semibold text-bodydark dark:text-white/70">Account Manager:</td>
            <td>${job.ownerRH}</td>
          </tr>
          <tr>
    ...
    <div class="flex space-x-2 items-center">
      <!-- Update Icon -->
      <a href="javascript:void(0)" onclick="get_job(${job.id})" class="text-blue-500 hover:text-blue-700 transition">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 4.172a4 4 0 015.656 5.656L8 19H4v-4L13.828 4.172z"></path>
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 14l2 2 2-2-2-2-2 2z"></path>
        </svg>
      ...
      <!-- Delete Icon -->
      <a style="display: ${is_superuser === "true" ? 'inline' : 'none'};" href="javascript:void(0)" onclick="deleteJob('${job.id}')" class="text-red-500 hover:text-red-700 transition">
        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
        </svg>
      </a>
