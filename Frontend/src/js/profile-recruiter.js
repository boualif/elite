...
function load_profile() {
    ...
    fetch('profileRecruiterContent.html')
        ...
        .then(data => {
            ...
            // if(is_superuser === "true"){
             // Only show edit button for superusers
             const is_superuser = localStorage.getItem("role");
            ...
            if (is_superuser === "true") {
                ...
                // Remove or hide edit button for non-admin users
...
// Modify your get_activities function to extract the number
    ...
    apiClient.get(url,
        ...
        .then(response => {
            ...
            data.forEach(element => {
                if (element.description.includes('added') && element.description.includes('candidates')) {
                    const match = element.description.match(/added (\d+) candidates/);
                    if (match && match[1]) {
                        totalCandidates += parseInt(match[1]);
                    }
                }
            
            localStorage.setItem('totalCandidates', totalCandidates.toString());
                ...
                container.innerHTML = `
                    <div class="my-6 -ml-1.5 flex w-full items-center">
                        <div class="z-10 w-1/12">
                            <div class="h-3.5 w-3.5 rounded-full bg-blue-600"></div>
                        </div>
                        <div class="w-11/12">
                            <p class="text-sm">${description}
                                ${id ? `<a onclick="handleAction('${ref}', '${id}')" 
                                   class="font-bold text-blue-600 cursor-pointer">${ref}</a>` : ''}
                            </p>
            ...
            // Store the total number of candidates for the table
            localStorage.setItem('totalCandidates', totalCandidates.toString());
            
            // Update the number in the table if it exists on the page
            const candidateCountCell = document.querySelector('td:nth-child(5) p');
            if (candidateCountCell) {
                candidateCountCell.textContent = totalCandidates;
            }
        })
        .catch(error => {
...
function handleAction(action, id) {
    console.log('Action:', action, 'ID:', id);  // Add this debug line
    switch (action) {
        case 'candidate':
            ...
            break;
        case 'client':
            apiClient.get(`/api/get-client/${id}/`, {  // Updated URL
                withCredentials: true,
            })
            .then(function (response) {
            ...
            break;
        case 'job':
            apiClient.get(`/api/get-job/${id}/`, {  // Updated URL
                withCredentials: true,
            })
            .then((response) => {
            ...
            break;
        case 'lead':
            apiClient.get(`/api/get-client/${id}/`, {  // Updated URL for leads
                withCredentials: true,
            })
            .then(function (response) {
                const clientData = response.data;
